...
export function Dialog({ open, onOpenChange, children }: DialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      />
      <div className="relative z-50 border border-border bg-card rounded-lg shadow-lg max-w-md w-full mx-4 max-h-[85vh] flex flex-col">
        {children}
      </div>
    </div>
  );
...
export function DialogHeader({ children }: DialogHeaderProps) {
  return <div className="px-4 py-2 shadow-sm bg-accent/5 shrink-0">{children}</div>;
}
...
export function DialogFooter({ children }: DialogFooterProps) {
  return (
    <div className="flex gap-2 justify-end px-6 pb-6 pt-4 shrink-0">{children}</div>
  );
}
