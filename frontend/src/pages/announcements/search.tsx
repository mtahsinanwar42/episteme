...
import type { Announcement, GetAnnouncementsParams } from "@/models/announcement";
...
const STATUS_OPTIONS = [
  { value: AnnouncementStatus.UPCOMING, label: "Upcoming" },
  { value: AnnouncementStatus.ONGOING, label: "Ongoing" },
  { value: AnnouncementStatus.COMPLETED, label: "Completed" },
  { value: AnnouncementStatus.DELETED, label: "Deleted" },
];
...
export default function AnnouncementSearch() {
  ...
  const [searchParams, setSearchParams] = useState<GetAnnouncementsParams | null>(null);
  ...
  const {
    ...
    queryKey: ["announcements", "search", queryParams],
    queryFn: () => announcementService.getAnnouncements(queryParams!),
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex gap-4 place-self-center">
            <Link to={`/announcements/${row?.original?.id}`}>
              <Eye className="size-4 text-foreground hover:text-foreground/80 cursor-pointer" />
            </Link>
            {isAdmin && status !== AnnouncementStatus.DELETED && (
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        <PageTitle title="Advanced Search - Announcements" />
        <PageSubTitle text="Search announcements using advanced filters" />
      </div>
