...
import type { Blog, GetBlogsParams } from "@/models/blog";
...
const STATUS_OPTIONS = [
  ...
  { value: BlogStatus.DELETED, label: "Deleted" },
];
...
export default function BlogSearch() {
  ...
  const statusOptions = isAdmin
    ? STATUS_OPTIONS
    : STATUS_OPTIONS.filter((option) => option.value === BlogStatus.PUBLISHED);
  ...
  const [searchParams, setSearchParams] = useState<GetBlogsParams | null>(null);
  ...
  const {
    ...
    queryKey: ["blogs", "search", queryParams],
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex gap-4 place-self-center">
            ...
            {isAdmin && status !== BlogStatus.DELETED && (
              <Link to={`/blogs/edit/${row?.original?.id}`}>
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        <PageTitle title="Advanced Search - Blogs" />
        <PageSubTitle text="Search blogs using advanced filters" />
      </div>
